...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { RelayInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw RelayInvalidDataException("`$name` is not set")
            is JsonNull -> throw RelayInvalidDataException("`$name` is null")
            else -> throw RelayInvalidDataException("`$name` is invalid, received $this")
        }
